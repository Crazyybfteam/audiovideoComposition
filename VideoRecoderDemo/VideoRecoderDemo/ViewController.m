...
-   (void)musicCompression:(NSURL*)audioUrl
{
    ...
    NSData  *aacData    =   [NSData dataWithContentsOfURL:audioUrl];
    
   NSData   *amrData    =   EncodeWAVEToAMR(aacData, 1, 16);
    NSString* path = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject]
                      stringByAppendingPathComponent:@"music.amr"];
    BOOL isfinish   =   [amrData writeToFile:path atomically:YES];
    if (isfinish) {
           NSLog(@"压缩后大小 %ldMB",(long)[self getFileSize:path]);
    }
